...
import {
  C, FF,
  uid, fmtEuro, nowTime, saveFile,
  todayISO, addDaysISO, fmtDate, timeAgo,
  readLS, writeLS,
  useTick, useWindowWidth,
  useToast,
  Surface, Btn, Tag, InputField, SelectField, Stepper, QRCanvas, Modal,
  SectionTitle, PageTitle, KPICard, playBeep,
} from "./shared/ui.jsx";
...
/* L'aiguillage entre verticales est fait par Root.jsx ; ici on ne reconnaît
   que le slug hôtelier, pour ne pas capter les tables du restaurant. */
function parseRoute() {
  const h = window.location.hash.match(/^#\/r\/demo-hotel\/t\/(\d+)/);
  if (h) return { view: "portal", room: Number(h[1]) };
  const base = (import.meta.env.BASE_URL || "/").replace(/\/$/, "");
  let path = window.location.pathname;
  if (base && path.startsWith(base)) path = path.slice(base.length);
  const m = path.match(/^\/r\/demo-hotel\/t\/(\d+)/);
  if (m) return { view: "portal", room: Number(m[1]) };
  return { view: "shell" };
}

/* ToastProvider et GlobalStyles sont fournis par Root.jsx. */
export default function App() {
  ...
  return route.view === "portal" ? <RoomPortal room={route.room} /> : <Shell />;
}
